...
CREATE TABLE tour (
    ...
    commission_value NUMERIC(10,2) CHECK(commission_value > 0),
...
CREATE TABLE booking (
    ...
    manual_discount NUMERIC(10,2) DEFAULT 0.00 CHECK(manual_discount >= 0),
...
ALTER TABLE _user_permission ADD CONSTRAINT fk_user_permission_permission
    FOREIGN KEY (permission_id) REFERENCES permission (permission_id) ON DELETE CASCADE;
 
ALTER TABLE _user_permission ADD CONSTRAINT fk_user_permission_user
    FOREIGN KEY (user_id) REFERENCES users (user_id) ON DELETE CASCADE;

ALTER TABLE tour ADD CONSTRAINT fk_tour_category
    FOREIGN KEY (category_id) REFERENCES category_tour (category_id) ON DELETE RESTRICT;

ALTER TABLE booking ADD CONSTRAINT fk_booking_tour
...
ALTER TABLE booking ADD CONSTRAINT fk_booking_pickup
    FOREIGN KEY (pickup_id) REFERENCES pickup_location (pickup_id) ON DELETE RESTRICT;
 
ALTER TABLE group_member ADD CONSTRAINT fk_group_member_booking
    FOREIGN KEY (booking_id) REFERENCES booking (booking_id) ON DELETE CASCADE;
 
ALTER TABLE payment ADD CONSTRAINT fk_payment_booking
    FOREIGN KEY (booking_id) REFERENCES booking (booking_id) ON DELETE CASCADE;
 
ALTER TABLE refund_request ADD CONSTRAINT fk_refund_booking
    FOREIGN KEY (booking_id) REFERENCES booking (booking_id) ON DELETE CASCADE;
 
ALTER TABLE refund_request ADD CONSTRAINT fk_refund_user
    FOREIGN KEY (user_id) REFERENCES users (user_id) ON DELETE RESTRICT;
 
ALTER TABLE status_history ADD CONSTRAINT fk_status_history_booking
    FOREIGN KEY (booking_id) REFERENCES booking (booking_id) ON DELETE CASCADE;
