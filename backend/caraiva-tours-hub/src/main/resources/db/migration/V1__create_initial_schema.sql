...
CREATE TABLE booking (
     ...
     manual_discount NUMERIC(10,2) DEFAULT 0.00 CHECK (
         manual_discount >= 0
             AND manual_discount <= total_price_snapshot
         ),
     ...
     current_status booking_status_enum NOT NULL,
     created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP NOT NULL,
...
ALTER TABLE refund_request ADD CONSTRAINT fk_refund_requested_by_user FOREIGN KEY (requested_by_user_id) REFERENCES users(user_id) ON DELETE RESTRICT;
...
CREATE INDEX idx_booking_status ON booking(current_status);
