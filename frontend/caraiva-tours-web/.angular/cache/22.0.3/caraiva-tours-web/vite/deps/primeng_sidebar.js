...
var k = (t, o, e) => new Promise((i, u) => {
  ...
  }, h = (s2) => s2.done ? i(s2.value) : Promise.resolve(s2.value).then(a, M);
...
function R2(t, o) {
  let e = window.getComputedStyle(t), i = (d) => {
    let l = e[`${d}Duration`].split(", ").map(se), m = e[`${d}Delay`].split(", ").map(se);
    return m.length < l.length && m.length > 0 && (m = l.map((v, b) => m[b % m.length])), [m, l];
  }, [u, a] = i(E), [M, h] = i(p), s2 = Math.max(...a.map((d, l) => d + u[l])), f = Math.max(...h.map((d, l) => d + M[l])), r, n = 0, c = 0;
  return o === E ? s2 > 0 && (r = E, n = s2, c = a.length) : o === p ? f > 0 && (r = p, n = f, c = h.length) : (n = Math.max(s2, f), r = n > 0 ? s2 > f ? E : p : void 0, c = r ? r === E ? a.length : h.length : 0), { type: r, timeout: n, count: c };
}
function q(t, o) {
...
function g(t, o, e) {
  ...
  i && ue(t, W2(a, "height"), M ? e.height : e), u && ue(t, W2(a, "width"), M ? e.width : e);
}
function P(t, o) {
...
function dt(t, o) {
  ...
  h(o), g(t, e, "0px");
  let f = { enter: () => e.enter ? s2("enter") : Promise.resolve(), leave: () => e.leave ? s2("leave") : Promise.resolve(), cancel: () => {
    a == null || a(), a = null;
  }, update: (r, n) => {
...
function ot(t, o, e, i) {
  ...
  let { type: M, timeout: h, count: s2 } = R2(t, o);
  if (!M) return i(), () => {
  };
  let f = M + "end", r = 0, n = () => {
